...
{    
    ...
    {    
        ...
        // foreach(Application::where('status','Completed')->get() as $item){
        //     Application::find($item->id)->update([
        ...
        foreach ($project as $item) {
            ...
            $array['status'] = $current_status->status ?? 'Pending';
    ...
    public function create()
    {
        ...
        try {            
            ...
            Project::create($req);
            DB::commit();
            return response()->json(['status' => true, 'message' => 'Projects created successfully!', 'data' => []]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function application($project_id)
    {
        ...
        return response()->json(['status' => true, 'message' => 'Applications fetched successfully!', 'page' => $page, 'data' => $data]);
    }
    ...
    public function completed($id){
       ...
       try{            
            ...
            $application = Application::where('project_id', $id)->where('user_id',authId())->where('status','Approved')->first();
            if(!$application){
