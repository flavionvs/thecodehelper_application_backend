   ...
   <body style="background-color: rgb(38 167 81)">
        <div class="form">                  
            <div class="form-small"; style="padding: 0px">
                ...
                <h1 class="h1-font">Meeting Booked<h1>
                <p class="p-font">Hey <b>{{$user->first_name}}</b>, Your meeting is scheduled in the room named <b>{{$booking->room->name}}</b> on <b>{{dateFormat($booking->date)}}</b> from <b>{{date('h:i A', strtotime($booking->from_time))}}</b> to <b>{{date('h:i A', strtotime($booking->to_time))}}</b>. You can cancel the meeting up to 30 minutes before the starting time using the below button.</p>                
